// OKLCH / Oklab color space conversions — WGSL port of color-oklch.glsl
// Based on Björn Ottosson's Oklab: https://bottosson.github.io/posts/oklab/
// Note: WGSL pow() only works on positive values, so we use cbrt() helper for cube roots

fn cbrt(x: f32) -> f32 {
  return sign(x) * pow(abs(x), 1.0 / 3.0);
}

fn rgb2oklab(c: vec3<f32>) -> vec3<f32> {
  let l = 0.4122214708 * c.r + 0.5363325363 * c.g + 0.0514459929 * c.b;
  let m = 0.2119034982 * c.r + 0.6806995451 * c.g + 0.1073969566 * c.b;
  let s = 0.0883024619 * c.r + 0.2817188376 * c.g + 0.6299787005 * c.b;

  let l_ = cbrt(l);
  let m_ = cbrt(m);
  let s_ = cbrt(s);

  return vec3<f32>(
    0.2104542553 * l_ + 0.7936177850 * m_ - 0.0040720468 * s_,
    1.9779984951 * l_ - 2.4285922050 * m_ + 0.4505937099 * s_,
    0.0259040371 * l_ + 0.7827717662 * m_ - 0.8086757660 * s_
  );
}

fn oklab2rgb(lab: vec3<f32>) -> vec3<f32> {
  let l_ = lab.x + 0.3963377774 * lab.y + 0.2158037573 * lab.z;
  let m_ = lab.x - 0.1055613458 * lab.y - 0.0638541728 * lab.z;
  let s_ = lab.x - 0.0894841775 * lab.y - 1.2914855480 * lab.z;

  let l = l_ * l_ * l_;
  let m = m_ * m_ * m_;
  let s = s_ * s_ * s_;

  return vec3<f32>(
    4.0767416621 * l - 3.3077115913 * m + 0.2309699292 * s,
    -1.2684380046 * l + 2.6097574011 * m - 0.3413193965 * s,
    -0.0041960863 * l - 0.7034186147 * m + 1.7076147010 * s
  );
}

fn rgb2oklch(rgb: vec3<f32>) -> vec3<f32> {
  let lab = rgb2oklab(rgb);
  let L = lab.x;
  let C = length(lab.yz);
  let H = atan2(lab.z, lab.y);
  return vec3<f32>(L, C, H);
}

fn oklch2rgb(lch: vec3<f32>) -> vec3<f32> {
  let a = lch.y * cos(lch.z);
  let b = lch.y * sin(lch.z);
  return oklab2rgb(vec3<f32>(lch.x, a, b));
}

// Perceptually uniform color mixing in Oklab space
fn mix_oklab(rgb1: vec3<f32>, rgb2: vec3<f32>, t: f32) -> vec3<f32> {
  let lab1 = rgb2oklab(rgb1);
  let lab2 = rgb2oklab(rgb2);
  return oklab2rgb(mix(lab1, lab2, t));
}

// Mix in OKLCH space (preserves hue path)
fn mix_oklch(rgb1: vec3<f32>, rgb2: vec3<f32>, t: f32) -> vec3<f32> {
  let lch1 = rgb2oklch(rgb1);
  let lch2 = rgb2oklch(rgb2);

  // Handle hue interpolation (shortest path)
  let h1 = lch1.z;
  let h2 = lch2.z;
  var dh = h2 - h1;
  if (dh > 3.14159265) { dh -= 6.28318530; }
  if (dh < -3.14159265) { dh += 6.28318530; }

  let lch = vec3<f32>(
    mix(lch1.x, lch2.x, t),
    mix(lch1.y, lch2.y, t),
    h1 + dh * t
  );

  return oklch2rgb(lch);
}
